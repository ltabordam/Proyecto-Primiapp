...
def login():
    ...
    def registro():
        ...
        Label(window, text='Sé un PrimiAmigo', bg='#94B43B', fg='Black', width='300', height='3',
              font=("Bold", 20)).pack()
...
def init_window():
    ...
    Label(primera_ventana, text='Primiapp', bg='#94B43B', fg='Black', width='300', height='3',
    ...
    Button(primera_ventana, text='Temporizador', bg='#94B43B', fg='Black', width='20', height='1',
           command=temporizador).pack()
    Label(primera_ventana, text='', bg='#B1B2B0').pack()

    Button(primera_ventana, text='Recargar', bg='#94B43B', fg='Black', width='6', height='1',
           command=init_window).pack()
    Label(primera_ventana, text='', bg='#B1B2B0').pack()

    Button(primera_ventana, text='Salir', bg='#94B43B', fg='Black', width='5', height='1', command=sys.exit).pack()

    messagebox.showinfo(
        message='Para una mejor experiencia una vez iniciada sesión minimice la pantalla de ingreso, muchas gracias')
...
def calendar():
    ...
    def init_agendar():
        ...
        labelA = Label(root, text="¿Qué deseas \n guardar?", font=("Arial Bold", 15), width=35, bg='#A61C31')
        ...
        t = Text(root, width=30, height=10, font=("Arial Bold", 10))
        t.grid(row=4, column=0)

        label_3 = Label(root, text="", bg='#B1B2B0')
        label_3.grid(row=5, column=0)

        bE = Button(root, text="Guardar", font=("Arial Bold", 10), command=lambda: take_input(), bg='#A61C31')
    ...
    def init_agendado():
        ...
        def show_info():
            ...
            for recordatorio in texto:
                if validar_registro in recordatorio:
                    ...
                    if validar_etiqueta in recordatorio:
                        ...
                        print(recordatorio)
                        if recordatorio[0:8] == str(cal.get_date()) or recordatorio[0:7] == str(
                                cal.get_date()) or recordatorio[0:6] == str(cal.get_date()):
            ...
            for festividad in texto_festivos:
                if festividad[0:8] == str(cal.get_date()) or festividad[0:7] == str(cal.get_date()) or festividad[
                                                                                                       0:6] == str(
                        cal.get_date()):
        ...
        root.geometry("390x350")
        root.config(bg='#B1B2B0')
        labelA = Label(root, text="Lo agendado!!", font=("Arial Bold", 15), width=35, bg='#94B43B')
        labelA.grid(row=0, column=0)
        ...
        t2 = Text(root, width=30, height=10, font=("Arial Bold", 10))
        t2.grid(row=4, column=0)

        label_3 = Label(root, text="", bg='#B1B2B0')
        label_3.grid(row=5, column=0)

        bE = Button(root, text="Muestrame", font=("Arial Bold", 10), command=lambda: show_info(), bg='#94B43B')
        bE.grid(row=6, column=0)
...
def window_notes():
    ...
    def menu_notas():
        ...
        Label(notas, text='Bienvenido a la sección' + '\n' + 'de Notas PrimiAmigo', bg='#A61C31', width='300',
              height='3', font=('Arial Bold', 15)).pack()
    ...
    def calculo_rapido():
        ...
        Label(calculo, text='Bienvenido a la sección de' + '\n' + 'Cálculo Rápido PrimiAmigo', bg='#A61C31',
              width='300', height='3', font=('Arial Bold', 15)).pack()
    ...
    def entrada_datos():
        ...
        Label(registro, text='Bienvenido PrimiAmigo', bg='#A61C31', width='300', height='3', font=('Arial Bold', 15)).pack()
        Label(text='', bg='#595A5C')
        ...
        messagebox.showinfo(
            message="Digita las notas unicamente separadas por coma (,). \n Guarda tu materia de una manera que la recuerdes facilmente",
    ...
    def buscar_notas():
        ...
        Label(reporte, text='Bienvenido PrimiAmigo', bg='#A61C31', width='300', height='3', font=('Arial Bold', 15)).pack()
        Label(text='', bg='#595A5C')
        ...
        messagebox.showinfo(message="Digita el nombre de tu materia de la misma manera que la guardaste",
                            title='Funcionamiento')
    ...
    def ponderado_de_notas():
        ...
        if ponderado_de_notas > nota_pasar and ponderado_de_notas >= 4.5:
            messagebox.showinfo(message=str(round(ponderado_de_notas, 2)) + ' ' + 'Fresco pelao que va pasando',
                                title='Melo')
        elif ponderado_de_notas > nota_pasar and 4.5 > ponderado_de_notas >= 4:
            messagebox.showinfo(message=str(round(ponderado_de_notas, 4)) + ' ' + 'Vas pasando,pero se puede mejorar',
                                title='Tu tranquilo')
        elif ponderado_de_notas >= nota_pasar and 4 > ponderado_de_notas >= 3:
            messagebox.showinfo(
                message=str(round(ponderado_de_notas, 4)) + ' ' + 'Raspando raspando te puede ir afectando',
                title='Ojito')
    ...
    def promedio_reporte():
        ...
        messagebox.showinfo(message='El ponderado de esta materia es:' + '\n' + str(ponderado),
                            title='Ponderado' + ' ' + str(cod_materia))
...
def temporizador():
    ...
    Lh = Label(root, font=("Arial Bold ", 30), fg='white', background="#466B3F")
    Lh.grid(column=3, row=1, padx=3)
    Ln = Label(root, font=("Arial Bold ", 30), fg='white', background="#466B3F")
    Ln.grid(column=4, row=1, padx=3)
    Lm = Label(root, font=("Arial Bold ", 30), fg='white', background="#466B3F")
    Lm.grid(column=5, row=1, padx=3)
    Ln2 = Label(root, font=("Arial Bold ", 30), fg='white', background="#466B3F")
    Ln2.grid(column=6, row=1, padx=3)
    Ls = Label(root, font=("Arial Bold ", 30), fg='white', background="#466B3F")
    ...
    b3 = Button(root, text="Ver Registro", font=('Arial Bold', 10), width=5, command=tempo_registro,
                background='#595A5C')
...
def tempo_registro():
    ...
    messagebox.showinfo(
        message="""Al ingresar un periodo de tiempo hacerlo bajo un formato de dia_menor - dia_mayor, Gracias por su colaboracion""")
    ...
    ltitulo3 = Label(root2, font=('Arial Bold', 8), fg='white', text='Ingrese Dia o Periodo de tiempo',
                     background="#76232F")
...
def ver_minutos():
    ...
    if len(str(entrada_fecha.get()) + '-' + str(entrada_fecha2.get()) + '-' + str(entrada_fecha3.get())) == 10:
        for elemento in info:
            if usuario in elemento[0] and (
                    str(entrada_fecha.get()) + '-' + str(entrada_fecha2.get()) + '-' + str(entrada_fecha3.get())) in \
                    elemento[2] and contraseña.get() in elemento[1] and elemento[4] in str(entrada_materia.get()):
    ...
    if len((str(entrada_fecha.get()) + '-' + str(entrada_fecha2.get()) + '-' + str(entrada_fecha3.get()))) == 13:
        for elemento in info:
            if usuario in elemento[0] and int(
                    (str(entrada_fecha.get()) + '-' + str(entrada_fecha2.get()) + '-' + str(entrada_fecha3.get()))[
                    11:12]) >= int(elemento[2][8:9]) and int(
                    (str(entrada_fecha.get()) + '-' + str(entrada_fecha2.get()) + '-' + str(entrada_fecha3.get()))[
                    8:9]) <= int(elemento[2][8:9]) and contraseña.get() in elemento[1] and elemento[4] in str(
                    entrada_materia.get()):
                tiempo_de_estudio = tiempo_de_estudio + int(elemento[3])
        valor_final = 'Horas: ' + str(tiempo_de_estudio // 60) + '  Minutos: ' + str(tiempo_de_estudio % 60)
